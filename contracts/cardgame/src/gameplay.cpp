...
// Find the first empty slot in the hand// Find the first empty slot in the hand
  int first_empty_slot = -1;
  for (int card_slot = 0; card_slot < hand.size(); card_slot++)
  {
    auto id = hand[card_slot];
    if (card_dict.at(id).type == EMPTY)
    {
      first_empty_slot = card_slot;
      break;
    }
  }
...
int cardgame::ai_choose_card(const game& game_data)
{
  ...
  for (int card_counter = 0; card_counter < game_data.hand_ai.size(); card_counter++) {
    const auto ai_card_id = game_data.hand_ai[card_counter];
...
{
   int card_score = 0;
   for (int card_to_check = 0; card_to_check < hand_player.size(); card_to_check++) {
      const auto player_card_id = hand_player[card_to_check];
